...
const personas = [
  { name: "Teenager with anxiety in Christchurch", region: "Canterbury", age: 16, want: "unsure", needs: ["anxiety"], preferences: [], barriers: ["privacy"], goal: "find a safe first step" },
...
function score(provider, persona, type) {
  ...
  if (provider.region === "National") value += persona.barriers.includes("transport") || persona.preferences.includes("telehealth") ? 5 : -3;
  ...
  if (persona.preferences.includes("telehealth") && (tags.includes("telehealth") || provider.region === "National")) value += 8;
  if (!persona.preferences.includes("telehealth") && (tags.includes("telehealth") || provider.region === "National")) value -= 3;
...
function recommendations(persona) {
  ...
  const primary = providers
    ...
    .filter((provider) => provider.region === persona.region || provider.region === "National")
